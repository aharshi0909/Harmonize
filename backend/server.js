...
const app = express()
const statsFunc = require('./statsfn')
const historyFunc = require('./historyfn')
...
app.post('/updateStats', async (req, res) => {
  const { email, data, filename1, filename2 } = req.body
  if (!email || !data) {
    res.send('Please provide email and data')
    return
  }
  const result = await statsFunc.addStats(email, data)

  if (filename1 && filename2) {
    await historyFunc.addHistory(email, {
      original: filename1,
      checker: filename2,
      ...data
    })
...
app.get('/getHistory', async (req, res) => {
  const email = req.query.email
  if (!email) {
    res.send({ error: 'Please provide email' })
    return
  }
  const result = await historyFunc.getHistory(email)
  res.send(result)
})
...
app.get('/getStats', async (req, res) => {
  ...
  const result = await statsFunc.getStats(email)
  res.send(result)
})
